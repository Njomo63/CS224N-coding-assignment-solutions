...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        dm = self.n_features * self.embed_size
        embed_to_hidden_weight = torch.empty(dm, self.hidden_size)
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.index_select(self.embeddings, dim=0, index=w.view(-1)).view(w.shape[0], w.shape[1], -1).flatten(start_dim=1)
    ...
    def forward(self, w):
        ...
        linear = x @ self.embed_to_hidden_weight
        linear += self.embed_to_hidden_bias
        m = nn.ReLU()
        relu = m(linear)
        relu = self.dropout(relu)
        linear2 = relu @ self.hidden_to_logits_weight
        logits = linear2 + self.hidden_to_logits_bias
