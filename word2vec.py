...
def naiveSoftmaxLossAndGradient(
    ...
    gradCenterVec = outsideVectors.T@delta
...
def negSamplingLossAndGradient(
    ...
    uovc = sigmoid(score[outsideWordIdx])
    neg_samples = -score[negSampleWordIndices]
    unvc = sigmoid(neg_samples)
    loss = -np.log(uovc) - np.sum(np.log(unvc), axis=0) #mistake
    
    phase_1 = -(1-uovc)*outsideVectors[outsideWordIdx]
    phase_2 = outsideVectors[negSampleWordIndices].T@(1-unvc)
    gradCenterVec = phase_1 + phase_2
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = -(1-uovc)*centerWordVec
    for i, neg_idx in enumerate(negSampleWordIndices):
        gradOutsideVecs[neg_idx] += (1 - unvc[i]) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    centerWordVec = centerWordVectors[ind]
    for word in outsideWords:
        loss_1, gradCenterVecs_1, gradOutsideVectors_1 = word2vecLossAndGradient(centerWordVec, word2Ind[word], outsideVectors, dataset)
        loss += loss_1
        gradCenterVecs[ind] += gradCenterVecs_1
        gradOutsideVectors += gradOutsideVectors_1
    ### END YOUR CODE
